## RealtimeRejectChatRequest Conversion Report

### Function overview

- **Original name**: `tayab_funcRejectChatRequest`
- **Category**: `realtime` / Socket.IO messaging
- **Purpose**: Reject a pending chat request by sending a `chat-request-reject` Socket.IO event to the target user's active socket, subject to tenant, auth, connectivity, and presence checks, including an optional rejection reason.

### Original v3 implementation summary

- **Location**: `/backend/tayab_funcRejectChatRequest/index.js`
...
- **Behavior**:
  ...
  - Extracts the request body as an object (defensive check) and reads:
    - `toUserName` or `targetUserName`.
    - `reason` or `message`.
  ...
  - Normalizes usernames:
    ...
    - Prevents self-targeting: if equal, returns `400` with `{ error: "Cannot reject your own chat request" }`.
  ...
  - Writes to the Socket.IO output binding:
    - `context.bindings.socketOutput = { actionName: "sendToSocket", eventName: "chat-request-reject", parameters: [payload], socketId: String(toSocketId) }`.
  - Logs `"funcRejectChatRequest success"` with `{ fromUserName, toUserName: normalizedToUser, toSocketId, reason }`.
  - Returns `200` with `{ success: true }`.
  - On outer error: logs `"funcRejectChatRequest error"` and returns `500` with `{ error: "Internal server error" }`.

### v4 implementation summary

- **Location**: `/backend-v4/functions/realtime/reject-chat-request.js`.
- **Trigger**: HTTP (via `app.http`, method: `POST`, `authLevel: function`).
- **Azure Function name**: `RealtimeRejectChatRequest`.
- **Route**: `realtime/reject-chat-request` (invoked as `/api/realtime/reject-chat-request` by default).
- **Structure changes**:
  - Uses v4 `app.http` registration with handler `realtimeRejectChatRequestHandler`.
...
- **Logic**:
  - **Initialization**:
    ...
    - On failure: logs a warning (`Service initialization failed in RealtimeRejectChatRequest`) and continues.
  ...
  - **Authentication**:
    ...
    - On failure: logs a warning (`Authentication required for realtime reject-chat-request`) and returns `401` with `{ error: "Authentication required" }`.
  - **Input parsing and validation**:
    - Reads JSON via `await request.json()` inside a `try/catch`, defaulting to `{}` on parse errors.
    - Extracts `toUserName`/`targetUserName` and `reason`/`message`.
  ...
  - **Username normalization and self-targeting prevention**:
    ...
    - If equal: returns `400` with `{ error: "Cannot reject your own chat request" }`.
  ...
  - **Event payload and binding**:
    ...
    - Constructs `socketMessage = { actionName: "sendToSocket", eventName: "chat-request-reject", parameters: [payload], socketId: String(toSocketId) }`.
    - Writes the message via `context.extraOutputs.set(socketIOOutput, socketMessage)`.
  - **Response and logging**:
    - Logs `"RealtimeRejectChatRequest success"` with the same metadata as v3.
    - Returns `200` with `{ success: true }`.
  - **Error handling**:
    - On outer error: logs `"Internal error in RealtimeRejectChatRequest"` and returns `500` with `{ error: "Internal server error" }`.

### Route and binding changes

- **Original**:
  - Route: implicitly `tayab_funcRejectChatRequest` (e.g. `/api/tayab_funcRejectChatRequest`).
  - Bindings (from `function.json`):
    ...
    ```
- **New**:
  - Route: `realtime/reject-chat-request` (accessed as `/api/realtime/reject-chat-request`).
...
- **Preserved behavior**:
  ...
  - Prevention of self-targeted chat-reject operations.
  - Target user lookup by username within the tenant, socket ID retrieval, and online presence validation.
  - Socket.IO message format and semantics (`actionName: "sendToSocket"`, `eventName: "chat-request-reject"`, payload with `fromUserName` and `reason`).
...
- **Differences / clarifications**:
  ...
  - Service initialization (`initializeApp()`) is now invoked but is non-fatal; this aligns reject-chat-request with other v4 functions without changing external behavior.
  - The HTTP route has been normalized to `realtime/reject-chat-request`; clients must be updated accordingly (client-v4 provides a typed wrapper).
- **Edge cases**:
...
- **Integration tests should**:
  ...
  - Call `POST /api/realtime/reject-chat-request` with:
    - Headers:
      - `Authorization: Bearer <jwt for A>`.
      - `x-functions-key`, `x-tenant-domain`, `x-forsero-tenant-domain`.
    - Body: `{ "toUserName": "<B.username>", "reason": "<some reason>" }`.
  - Assert:
    - `200` with `{ success: true }` on the HTTP response.
    - A corresponding `chat-request-reject` event delivered to the mocked Socket.IO binding for socket `<B.socketId>` with the expected payload.
...
- The `reason` field is passed through verbatim; callers are responsible for constraining/validating its contents according to UX needs.
