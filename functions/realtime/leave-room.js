...
/**
 * Sends a leave-room event to a target user's active Socket.IO connection.
 *
 * v4 Azure Functions implementation of the legacy v3 `tayab_funcLeaveRoom`.
 *
 * Responsibilities:
 ...
 * - Emit a `room-leave` event to the target user's socket via the Socket.IO output binding.
 *
 ...
 * @returns {Promise<import('@azure/functions').HttpResponseInit>} HTTP response with leave status.
 */
async function realtimeLeaveRoomHandler(request, context) {
	let tenant = null;
	let auth = null;
	...
	try {
		...
		try {
			...
			context.log?.warn?.(
				'Service initialization failed in RealtimeLeaveRoom:',
		...
		try {
			...
			context.log?.warn?.(
				'Tenant resolution/validation failed in RealtimeLeaveRoom:',
				tenantError,
			);
		...
		try {
			...
			context.log?.warn?.(
				'Authentication required for realtime leave-room:',
		...
		// Prevent self-targeting (leaving own room).
		if (
			normalizedToUser &&
			normalizedFromUser &&
			...
			return {
				status: 400,
				jsonBody: {error: 'Cannot leave your own room'},
			};
		}
		...
		context.log?.info?.('RealtimeLeaveRoom success', {
			fromUserName,
		...
		context.log?.error?.('Internal error in RealtimeLeaveRoom:', error);
