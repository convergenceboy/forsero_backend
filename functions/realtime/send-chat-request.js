...
const CHAT_REQUEST_EVENT = 'chat-request';

/**
 * Send a chat request event to another user's active Socket.IO connection.
 *
 * v4 Azure Functions implementation of the legacy v3
 * `tayab_funcSendChatRequest`.
 *
 * Responsibilities:
 ...
 * - Emit a `chat-request` event to the target user's socket via the Socket.IO
 *   output binding, including optional encryption metadata.
 *
 ...
 * @returns {Promise<import('@azure/functions').HttpResponseInit>} HTTP response with chat-request status.
 */
async function realtimeSendChatRequestHandler(request, context) {
	let tenant = null;
	let auth = null;
	...
	try {
		...
		try {
			...
			context.log?.warn?.(
				'Service initialization failed in RealtimeSendChatRequest:',
		...
		try {
			...
			context.log?.warn?.(
				'Tenant resolution/validation failed in RealtimeSendChatRequest:',
				tenantError,
			);
		...
		try {
			...
			context.log?.warn?.(
				'Authentication required for realtime send-chat-request:',
		...
		// Prevent self-targeting (sending a chat request to self).
		if (
			normalizedToUser &&
			normalizedFromUser &&
			...
			return {
				status: 400,
				jsonBody: {error: 'Cannot send a chat request to yourself'},
			};
		}
		...
		context.log?.info?.('RealtimeSendChatRequest success', {
			fromUserName,
		...
		context.log?.error?.('Internal error in RealtimeSendChatRequest:', error);
